...
export default function walkExcel(time) {
  let data = [];
  ...
  for (const fileName of fileNames) {
    ...
    const workSheetsFromFile = xlsx.parse(
      resolve(__dirname, `../sheets/${fileName}`)
    );
    ...
    if (rwly && ggrq && ggh) {
      ...
      data = [
        ...data,
        ...genStandardExcel(titlesAndData.data, isField(titlesAndData.titles)),
      ];
    }
  }
  data = [Object.keys(rule), ...data];
  const buffer = xlsx.build([{ name: "sheet1", data }]);
  fs.writeFileSync(`./sheets/${time}不合格数据.xlsx`, buffer, { flag: "w" });
}
